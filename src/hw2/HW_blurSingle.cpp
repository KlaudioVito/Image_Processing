// ======================================================================
// Senior Design CSC59866 - Capstone 1
//
// Blurring.cpp - Blurring widget
//
// Written by: Klaudio Vito, 2016
// ======================================================================
//#include "filterSingle.cpp"

void
filterSingle(ChannelPtr<uchar>p1, int len, int steps, int kernelSize, ChannelPtr<uchar> p2);
...
//
void
HW_blurSingle(ImagePtr I1, int width, int height, ImagePtr I2)
{
	...
	for (int ch = 0; IP_getChannel(I1, ch, p1, type); ch++) {
		...
		for (int v = 0; v < h; v++, p1+=w, p2+=w) filterSingle(p1, w, 1, width, p2);	//filter horizontally one step at a time
		p2 -= (total - 1);														//go one step back

		IP_getChannel(I2, ch, end, type);
		for (int v = 0; v < w; v++, p2++, end++)  filterSingle(p2, h, w, height, end);//filter vertically every w steps
	}
	//m_blurred = I2;																//get blurred output image
}
