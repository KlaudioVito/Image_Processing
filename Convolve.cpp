...
enum { WSIZE, HSIZE, STEPX, STEPY, WEIGHTS, SAMPLER };
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Convolve::Convolve:
//
// Constructor.
...
Convolve::load()
{
	...
	if (w % 2 == 0 || h % 2 == 0) {
		QMessageBox msgBox;
		msgBox.setWindowTitle("Kernel size error.");
		msgBox.setIcon(QMessageBox::Information);
		msgBox.setStandardButtons(QMessageBox::Ok);
		msgBox.setText("Kernel size must be odd.");
		msgBox.exec();
...
Convolve::gpuProgram(int pass) 
{
	...
	// get pointer to kernel values
	IP_getChannel(m_kernel, 0, p, type);
	//get kernel weights, left-right then top-bottom
	int k = 0;
	for (int i = 0; i < w; ++i)
		for (int j = 0; j < h; ++j)
			weights[k++] = *p++;
	...
	glUniform1fv(m_uniform[pass][WEIGHTS], 100, weights);
